        ...
        for (Time[] timeWindow : timeWindows) {
            ...
            if (FlinkParameters.getParameters().getBoolean("use.window.chaining", false)) {
                if (currentAggregabileCrossroadsStream == null) {
                    ...
                    // Key by the crossroads id
                    KeyedStream<JNAggregabileCrossroads, String> keyedTimestampedAggregabileCrossroadsStream = currentAggregabileCrossroadsStream.keyBy(new JNAggregabileCrossroadsIdKeySelector());
                    // Assign the pre-aggregation window
            ...
            // Key by random id
            KeyedStream<Identified<JNCrossroads>, String> keyedTimestampedCrossroadsStream = crossroadsStream.map(new JNObjectToRandomIdentifiedMapFunction(FlinkParameters.getParameters().getInt("flink.parallelism", 4))).name("JNObjectToRandomIdentifiedMapFunction").keyBy(new JNIdentifiedIdKeySelector());
            ...
            // Assign sliding windows
            AllWindowedStream<JNCrossroadsLeaderboard, TimeWindow> windowedTimestampedPartialTop10CrossroadsLeaderboardStream = timeWindow[1].toMilliseconds() == 0 ? partialTop10CrossroadsLeaderboardStream.timeWindowAll(timeWindow[0]) : partialTop10CrossroadsLeaderboardStream.timeWindowAll(timeWindow[0], timeWindow[1]);
            ...
            // Assign sliding windows
            AllWindowedStream<JNMedian, TimeWindow> windowedTimestampedPartialCrossroadsMedianDataStream = timeWindow[1].toMilliseconds() == 0 ? partialCrossroadsMedianDataStream.timeWindowAll(timeWindow[0]) : partialCrossroadsMedianDataStream.timeWindowAll(timeWindow[0], timeWindow[1]);

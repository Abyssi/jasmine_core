...
if [ ! -z "$KAFKA_LISTENERS" ]; then
    #replace kafka_port config in config file
    echo "listeners: $KAFKA_LISTENERS --------> I'm goning to set it in config file"
    #use sed to find config variable field and replace its value
...
if [ ! -z "$KAFKA_ADVERTISED_LISTENERS" ]; then
    #replace kafka_port config in config file
    echo "advertised listeners: $KAFKA_ADVERTISED_LISTENERS --------> I'm goning to set it in config file"
    #use sed to find config variable field and replace its value
    sed -r -i "s/#(advertised.listeners)=(.*)/\1=$KAFKA_ADVERTISED_LISTENERS/g" $KAFKA_HOME/config/server.properties
fi

if [ ! -z "$KAFKA_LISTENER_SECURITY_PROTOCOL_MAP" ]; then
    #replace kafka_port config in config file
    echo "listener.security.protocol.map: $KAFKA_LISTENER_SECURITY_PROTOCOL_MAP --------> I'm goning to set it in config file"
    #use sed to find config variable field and replace its value
    sed -r -i "s/#(listener.security.protocol.map)=(.*)/\1=$KAFKA_LISTENER_SECURITY_PROTOCOL_MAP/g" $KAFKA_HOME/config/server.properties
fi

if [ ! -z "$KAFKA_INTER_BROKER_LISTENER_NAME" ]; then
    #replace kafka_port config in config file
    echo "inter.broker.listener.name: $KAFKA_INTER_BROKER_LISTENER_NAME --------> I'm goning to set it in config file"
    #use sed to find config variable field and replace its value
    sed -r -i "s/#(inter.broker.listener.name)=(.*)/\1=$KAFKA_INTER_BROKER_LISTENER_NAME/g" $KAFKA_HOME/config/server.properties
fi
